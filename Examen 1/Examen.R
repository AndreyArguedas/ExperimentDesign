...
# Tres factores: Algoritmo ERA, CFS y ULE
...
Summarize(Puntaje ~ Algoritmo, Data, digits = 3)

ERA <- Data$Puntaje[Data$Algoritmo == "ERA"]
CFS <- Data$Puntaje[Data$Algoritmo == "CFS"]
ULE <- Data$Puntaje[Data$Algoritmo == "ULE"]
MONO <- Data$Puntaje[Data$Algoritmo == "Monotonic"]

plotNormalHistogram(ERA)
plotNormalHistogram(CFS)
plotNormalHistogram(ULE)
plotNormalHistogram(MONO)
...
Sum = groupwiseMean(Puntaje ~ Algoritmo, Data, conf = 0.95, digits = 3, traditional = FALSE, percentile = TRUE)
...
model = lm(Puntaje~Algoritmo, Data)
summary(model)
